...
function adicionarOuAlterar(e){
	...
	var matri = document.getElementById('txtMatricula').value;

	var p = {
		nome : !nom ? "sem nome": nom, //mesmo que if(nom = ""){ nom = "sem nome";}
		matric : !matri ? "sem matricula" : matri,
...
function prepararAlterar(idRow){	
	...
	for(var i = 0; i < pessoas.length; i++){
		if(pessoas[i].Id == idRow){			
			...
			txtMatricula.value = pessoas[i].Matricula; //caso fosse tipo date toISOString().substring(0, 10);
